...
func _check_code():
	if "".join(current_code) == correct_code:
		...
		var lock_area = chest_node.get_node("InteractionZone")  # podaj dokładną ścieżkę do Area3D
		if lock_area:
...
func _close_ui():
	...
	if chest_node:
		chest_node.set_meta("saved_code", current_code.duplicate())
...
func setup_ui_for_player(player: CharacterBody3D, chest: Node3D):
	...
	if chest_node.has_meta("saved_code"):
		current_code = chest_node.get_meta("saved_code")
		_update_display()
		_highlight_selected()
